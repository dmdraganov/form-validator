import { validateSnils, applySnilsMask } from '../../src/features/snilsValidation';
...
describe('SNILS Mask', () => {
  it('should apply a SNILS mask to a string', () => {
    const event = {
      target: {
        value: '11223344595',
      },
    ...
    expect((event.target as HTMLInputElement).value).toBe('112-233-445 95');
  });

  it('should handle partial SNILS input', () => {
    const event = {
      target: {
        value: '112233',
      },
    ...
    applySnilsMask(event);
    expect((event.target as HTMLInputElement).value).toBe('112-233');
  });
});
